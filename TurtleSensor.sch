EESchema Schematic File Version 2
LIBS:power
LIBS:device
LIBS:transistors
LIBS:conn
LIBS:linear
LIBS:regul
LIBS:74xx
LIBS:cmos4000
LIBS:adc-dac
LIBS:memory
LIBS:xilinx
LIBS:microcontrollers
LIBS:dsp
LIBS:microchip
LIBS:analog_switches
LIBS:motorola
LIBS:texas
LIBS:intel
LIBS:audio
LIBS:interface
LIBS:digital-audio
LIBS:philips
LIBS:display
LIBS:cypress
LIBS:siliconi
LIBS:opto
LIBS:atmel
LIBS:contrib
LIBS:valves
LIBS:SI7021
LIBS:W25Q32FVSSIG
LIBS:TurtleSensor-cache
EELAYER 25 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 1
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L Battery-RESCUE-TurtleSensor BT1
U 1 1 5622E668
P 1100 2750
F 0 "BT1" H 1200 2800 50  0000 L CNN
F 1 "534-3039" H 1200 2700 50  0000 L CNN
F 2 "footprints:2477Holder" V 1100 2790 60  0001 C CNN
F 3 "" V 1100 2790 60  0000 C CNN
	1    1100 2750
	1    0    0    -1  
$EndComp
Wire Wire Line
	1100 2600 1100 1550
Wire Wire Line
	1100 1550 2400 1550
Wire Wire Line
	2150 1550 2150 2450
Connection ~ 2150 1550
Wire Wire Line
	2150 1850 2400 1850
Wire Wire Line
	1100 3650 1100 2900
Connection ~ 2150 1850
Wire Wire Line
	2150 2750 2150 3950
Wire Wire Line
	2150 3750 2400 3750
Connection ~ 2150 3750
$Comp
L LED-RESCUE-TurtleSensor D1
U 1 1 5622E8A4
P 4900 1650
F 0 "D1" H 4900 1750 50  0000 C CNN
F 1 "5988110107F" H 4900 1550 50  0000 C CNN
F 2 "LEDs:LED_0805" H 4900 1650 60  0001 C CNN
F 3 "http://www.newark.com/dialight/5988110107f/led-red-1-25mm-x-1-4mm-40mcd-642nm/dp/75R0581" H 4900 1650 60  0000 C CNN
	1    4900 1650
	-1   0    0    1   
$EndComp
$Comp
L R R1
U 1 1 5622EA9F
P 5350 1650
F 0 "R1" V 5430 1650 50  0000 C CNN
F 1 "33Ω" V 5350 1650 50  0000 C CNN
F 2 "Resistors_SMD:R_0805_HandSoldering" V 5280 1650 30  0001 C CNN
F 3 "" H 5350 1650 30  0000 C CNN
	1    5350 1650
	0    1    1    0   
$EndComp
Wire Wire Line
	5600 1700 5600 1650
Wire Wire Line
	5600 1650 5500 1650
Wire Wire Line
	5200 1650 5100 1650
$Comp
L Crystal 32kHz1
U 1 1 56B6E109
P 5200 2200
F 0 "32kHz1" H 5200 2350 50  0000 C CNN
F 1 "TXC 9HT11-32.768KDZF-T" H 5200 2050 50  0000 C CNN
F 2 "Resistors_SMD:R_0805_HandSoldering" H 5200 2200 50  0001 C CNN
F 3 "https://octopart.com/9ht11-32.768kdzf-t-txc-18803093" H 5200 2200 50  0000 C CNN
	1    5200 2200
	0    1    1    0   
$EndComp
$Comp
L C C3
U 1 1 56B6E156
P 5650 2050
F 0 "C3" H 5675 2150 50  0000 L CNN
F 1 "12.5pF" H 5675 1950 50  0000 L CNN
F 2 "Capacitors_SMD:C_0805_HandSoldering" H 5688 1900 50  0001 C CNN
F 3 "" H 5650 2050 50  0000 C CNN
	1    5650 2050
	0    1    1    0   
$EndComp
$Comp
L C C4
U 1 1 56B6E19B
P 5650 2350
F 0 "C4" H 5675 2450 50  0000 L CNN
F 1 "12.5pF" H 5675 2250 50  0000 L CNN
F 2 "Capacitors_SMD:C_0805_HandSoldering" H 5688 2200 50  0001 C CNN
F 3 "" H 5650 2350 50  0000 C CNN
	1    5650 2350
	0    1    1    0   
$EndComp
$Comp
L GND #PWR01
U 1 1 56B6E1C2
P 5950 2500
F 0 "#PWR01" H 5950 2250 50  0001 C CNN
F 1 "GND" H 5950 2350 50  0000 C CNN
F 2 "" H 5950 2500 50  0000 C CNN
F 3 "" H 5950 2500 50  0000 C CNN
	1    5950 2500
	1    0    0    -1  
$EndComp
Wire Wire Line
	5800 2050 5950 2050
Wire Wire Line
	5950 2050 5950 2500
Wire Wire Line
	5800 2350 5950 2350
Connection ~ 5950 2350
Wire Wire Line
	4800 2050 5500 2050
Wire Wire Line
	4800 2350 5500 2350
Wire Wire Line
	4300 2150 4800 2150
Connection ~ 5200 2050
Wire Wire Line
	4300 2250 4800 2250
Connection ~ 5200 2350
Wire Wire Line
	6350 2250 6450 2250
Wire Wire Line
	6350 2250 6350 2050
$Comp
L GND #PWR02
U 1 1 56C0C95E
P 6350 2700
F 0 "#PWR02" H 6350 2450 50  0001 C CNN
F 1 "GND" H 6350 2550 50  0000 C CNN
F 2 "" H 6350 2700 50  0000 C CNN
F 3 "" H 6350 2700 50  0000 C CNN
	1    6350 2700
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR03
U 1 1 56C0CFBC
P 2150 3950
F 0 "#PWR03" H 2150 3700 50  0001 C CNN
F 1 "GND" H 2150 3800 50  0000 C CNN
F 2 "" H 2150 3950 50  0000 C CNN
F 3 "" H 2150 3950 50  0000 C CNN
	1    2150 3950
	1    0    0    -1  
$EndComp
$Comp
L +BATT #PWR04
U 1 1 56C0D0A8
P 1100 1550
F 0 "#PWR04" H 1100 1400 50  0001 C CNN
F 1 "+BATT" H 1100 1690 50  0000 C CNN
F 2 "" H 1100 1550 50  0000 C CNN
F 3 "" H 1100 1550 50  0000 C CNN
	1    1100 1550
	1    0    0    -1  
$EndComp
$Comp
L +BATT #PWR05
U 1 1 56C0D5FA
P 8250 3150
F 0 "#PWR05" H 8250 3000 50  0001 C CNN
F 1 "+BATT" H 8250 3290 50  0000 C CNN
F 2 "" H 8250 3150 50  0000 C CNN
F 3 "" H 8250 3150 50  0000 C CNN
	1    8250 3150
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR06
U 1 1 56C0D622
P 8250 3900
F 0 "#PWR06" H 8250 3650 50  0001 C CNN
F 1 "GND" H 8250 3750 50  0000 C CNN
F 2 "" H 8250 3900 50  0000 C CNN
F 3 "" H 8250 3900 50  0000 C CNN
	1    8250 3900
	1    0    0    -1  
$EndComp
Text Label 7400 2250 0    60   ~ 0
SDA
Text Label 7400 2450 0    60   ~ 0
SCL
Text Label 4550 1850 0    60   ~ 0
MOSI
Text Label 4550 1950 0    60   ~ 0
MISO
Text Label 4550 2050 0    60   ~ 0
SCK
Wire Wire Line
	4800 2250 4800 2350
Wire Wire Line
	4800 2150 4800 2050
Text Label 4550 1750 0    60   ~ 0
~SS
Wire Wire Line
	4550 1750 4300 1750
Wire Wire Line
	4550 1850 4300 1850
Wire Wire Line
	4300 1950 4550 1950
Wire Wire Line
	4550 2050 4300 2050
Text Label 4500 2800 0    60   ~ 0
SDA
Text Label 4500 2900 0    60   ~ 0
SCL
Wire Wire Line
	4500 2800 4300 2800
Wire Wire Line
	4300 2900 4500 2900
Wire Wire Line
	1100 3650 2400 3650
Connection ~ 2150 3650
Wire Wire Line
	2150 2150 2400 2150
Connection ~ 2150 2150
Wire Wire Line
	2400 3850 2150 3850
Connection ~ 2150 3850
$Comp
L GND #PWR07
U 1 1 56C6BC27
P 5600 1700
F 0 "#PWR07" H 5600 1450 50  0001 C CNN
F 1 "GND" H 5600 1550 50  0000 C CNN
F 2 "" H 5600 1700 50  0000 C CNN
F 3 "" H 5600 1700 50  0000 C CNN
	1    5600 1700
	1    0    0    -1  
$EndComp
$Comp
L +BATT #PWR08
U 1 1 56C6BCDC
P 6350 2050
F 0 "#PWR08" H 6350 1900 50  0001 C CNN
F 1 "+BATT" H 6350 2190 50  0000 C CNN
F 2 "" H 6350 2050 50  0000 C CNN
F 3 "" H 6350 2050 50  0000 C CNN
	1    6350 2050
	1    0    0    -1  
$EndComp
Wire Wire Line
	4300 3000 4500 3000
Text Label 4500 3000 0    60   ~ 0
~RST
$Comp
L FTDIConnector J1
U 1 1 56DB7438
P 5550 6000
F 0 "J1" H 5300 6600 60  0000 C CNN
F 1 "FTDIConnector" H 5350 5350 60  0000 C CNN
F 2 "Pin_Headers:Pin_Header_Straight_1x06_Pitch2.54mm" H 5550 5650 60  0001 C CNN
F 3 "" H 5550 5650 60  0000 C CNN
	1    5550 6000
	1    0    0    -1  
$EndComp
Wire Wire Line
	5000 6100 4700 6100
Wire Wire Line
	5000 6250 4700 6250
Wire Wire Line
	4100 6400 5000 6400
Text Label 3400 6400 0    60   ~ 0
~RST
Text Label 4700 6250 0    60   ~ 0
TX
Text Label 4700 6100 0    60   ~ 0
RX
$Comp
L GND #PWR09
U 1 1 56DB7AC0
P 4550 6550
F 0 "#PWR09" H 4550 6300 50  0001 C CNN
F 1 "GND" H 4550 6400 50  0000 C CNN
F 2 "" H 4550 6550 50  0000 C CNN
F 3 "" H 4550 6550 50  0000 C CNN
	1    4550 6550
	1    0    0    -1  
$EndComp
$Comp
L +BATT #PWR010
U 1 1 56DB7B9E
P 4400 5500
F 0 "#PWR010" H 4400 5350 50  0001 C CNN
F 1 "+BATT" H 4400 5640 50  0000 C CNN
F 2 "" H 4400 5500 50  0000 C CNN
F 3 "" H 4400 5500 50  0000 C CNN
	1    4400 5500
	1    0    0    -1  
$EndComp
Wire Wire Line
	4300 3250 4500 3250
Wire Wire Line
	4300 3150 4500 3150
Text Label 4500 3150 0    60   ~ 0
RX
Text Label 4500 3250 0    60   ~ 0
TX
Wire Wire Line
	4550 6550 4550 5650
Wire Wire Line
	4550 5650 5000 5650
NoConn ~ 5000 5800
NoConn ~ 4300 1550
NoConn ~ 4300 2400
NoConn ~ 4300 2500
NoConn ~ 4300 2600
NoConn ~ 4300 2700
NoConn ~ 4300 3450
NoConn ~ 4300 3550
NoConn ~ 4300 3650
NoConn ~ 4300 3750
NoConn ~ 4300 3850
$Comp
L C C2
U 1 1 56DB8C77
P 3950 6400
F 0 "C2" H 3975 6500 50  0000 L CNN
F 1 "0.1uF" H 3975 6300 50  0000 L CNN
F 2 "Capacitors_SMD:C_0805_HandSoldering" H 3988 6250 50  0001 C CNN
F 3 "" H 3950 6400 50  0000 C CNN
	1    3950 6400
	0    1    1    0   
$EndComp
Wire Wire Line
	3400 6400 3800 6400
$Comp
L SI7021 U1
U 1 1 56EDF09F
P 6850 2350
F 0 "U1" H 6550 2570 50  0000 L CNN
F 1 "SI7021-A20-GM1R" H 6550 2060 50  0000 L CNN
F 2 "SI7021:DFN-6%2f3MM" H 6850 2350 50  0001 L CNN
F 3 "https://www.silabs.com/Support%20Documents%2FTechnicalDocs%2FSi7021-A20.pdf" H 6850 2350 60  0000 C CNN
	1    6850 2350
	1    0    0    -1  
$EndComp
Wire Wire Line
	6450 2450 6350 2450
Wire Wire Line
	6350 2450 6350 2700
Wire Wire Line
	7250 2450 7400 2450
Wire Wire Line
	7400 2250 7250 2250
Wire Wire Line
	8250 3400 8250 3900
Wire Wire Line
	8250 3250 8250 3150
$Comp
L ATMEGA328P-A IC1
U 1 1 56EE0075
P 3300 2650
F 0 "IC1" H 2550 3900 50  0000 L BNN
F 1 "ATMEGA328PB-A" H 3700 1250 50  0000 L BNN
F 2 "Housings_QFP:TQFP-32_7x7mm_Pitch0.8mm" H 3300 2650 50  0000 C CIN
F 3 "http://www.atmel.com/Images/Atmel-42397-8-bit-AVR-Microcontroller-ATmega328PB_Datasheet.pdf" H 3300 2650 50  0000 C CNN
	1    3300 2650
	1    0    0    -1  
$EndComp
Wire Wire Line
	2400 1650 2150 1650
Connection ~ 2150 1650
NoConn ~ 2400 2900
NoConn ~ 2400 3000
$Comp
L M24512 U2
U 1 1 575F3396
P 7400 3600
F 0 "U2" H 7450 3950 60  0000 C CNN
F 1 "M24M01-RMN6TP" H 7400 3450 60  0000 C CNN
F 2 "SMD_Packages:SOIC-8-N" H 7400 3600 60  0001 C CNN
F 3 "www.st.com/resource/en/datasheet/m24m01-df.pdf" H 7400 3600 60  0000 C CNN
	1    7400 3600
	1    0    0    -1  
$EndComp
Wire Wire Line
	8250 3250 8100 3250
Wire Wire Line
	8100 3400 8250 3400
Wire Wire Line
	6750 3250 6600 3250
Wire Wire Line
	6600 3250 6600 3900
Wire Wire Line
	6600 3700 6750 3700
Wire Wire Line
	6750 3550 6600 3550
Connection ~ 6600 3550
Wire Wire Line
	6600 3400 6750 3400
Connection ~ 6600 3400
$Comp
L GND #PWR011
U 1 1 575F35E6
P 6600 3900
F 0 "#PWR011" H 6600 3650 50  0001 C CNN
F 1 "GND" H 6600 3750 50  0000 C CNN
F 2 "" H 6600 3900 50  0000 C CNN
F 3 "" H 6600 3900 50  0000 C CNN
	1    6600 3900
	1    0    0    -1  
$EndComp
Connection ~ 6600 3700
Wire Wire Line
	8100 3550 8350 3550
Wire Wire Line
	8350 3700 8100 3700
Text Label 8350 3550 0    60   ~ 0
SCL
Text Label 8350 3700 0    60   ~ 0
SDA
$Comp
L +BATT #PWR012
U 1 1 5768BC03
P 8500 1600
F 0 "#PWR012" H 8500 1450 50  0001 C CNN
F 1 "+BATT" H 8500 1740 50  0000 C CNN
F 2 "" H 8500 1600 50  0000 C CNN
F 3 "" H 8500 1600 50  0000 C CNN
	1    8500 1600
	1    0    0    -1  
$EndComp
$Comp
L R R2
U 1 1 5768BC8F
P 8300 1850
F 0 "R2" V 8380 1850 50  0000 C CNN
F 1 "4k7" V 8300 1850 50  0000 C CNN
F 2 "Resistors_SMD:R_0805_HandSoldering" V 8230 1850 50  0001 C CNN
F 3 "" H 8300 1850 50  0000 C CNN
	1    8300 1850
	1    0    0    -1  
$EndComp
$Comp
L R R3
U 1 1 5768BDF6
P 8700 1850
F 0 "R3" V 8780 1850 50  0000 C CNN
F 1 "4k7" V 8700 1850 50  0000 C CNN
F 2 "Resistors_SMD:R_0805_HandSoldering" V 8630 1850 50  0001 C CNN
F 3 "" H 8700 1850 50  0000 C CNN
	1    8700 1850
	1    0    0    -1  
$EndComp
Wire Wire Line
	8300 1700 8700 1700
Wire Wire Line
	8500 1600 8500 1700
Connection ~ 8500 1700
Text Label 8300 2100 0    60   ~ 0
SDA
Text Label 8700 2100 0    60   ~ 0
SCL
Wire Wire Line
	8700 2000 8700 2100
Wire Wire Line
	8300 2000 8300 2100
$Comp
L R 10k1
U 1 1 57C323B4
P 3950 5950
F 0 "10k1" V 4030 5950 50  0000 C CNN
F 1 "10k" V 3950 5950 50  0000 C CNN
F 2 "Resistors_SMD:R_0805_HandSoldering" V 3880 5950 50  0001 C CNN
F 3 "" H 3950 5950 50  0000 C CNN
	1    3950 5950
	0    1    1    0   
$EndComp
Wire Wire Line
	4700 1650 4500 1650
Text Label 4500 1650 0    60   ~ 0
SCK
$Comp
L C C5
U 1 1 57FE8071
P 2150 2600
F 0 "C5" H 2175 2700 50  0000 L CNN
F 1 "10µF" H 2175 2500 50  0000 L CNN
F 2 "Capacitors_SMD:C_0805_HandSoldering" H 2188 2450 50  0001 C CNN
F 3 "" H 2150 2600 50  0000 C CNN
	1    2150 2600
	1    0    0    -1  
$EndComp
$Comp
L C C1
U 1 1 57FE81CA
P 1850 2600
F 0 "C1" H 1875 2700 50  0000 L CNN
F 1 "0.1µF" H 1875 2500 50  0000 L CNN
F 2 "Capacitors_SMD:C_0805_HandSoldering" H 1888 2450 50  0001 C CNN
F 3 "" H 1850 2600 50  0000 C CNN
	1    1850 2600
	1    0    0    -1  
$EndComp
Wire Wire Line
	1850 2750 1850 3650
Connection ~ 1850 3650
Wire Wire Line
	1850 2450 1850 1550
Connection ~ 1850 1550
Wire Wire Line
	4100 5950 5000 5950
Wire Wire Line
	4300 3350 4500 3350
Text Label 4500 3350 0    60   ~ 0
RX
Wire Wire Line
	3800 5950 3700 5950
Wire Wire Line
	3700 5950 3700 6400
Connection ~ 3700 6400
Wire Wire Line
	4400 5500 4400 5950
Connection ~ 4400 5950
$EndSCHEMATC
